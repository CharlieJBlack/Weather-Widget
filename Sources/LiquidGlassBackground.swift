...
public struct LiquidGlassBackground<Content: View>: NSViewRepresentable {
    ...
    /// - Parameters:
    ///   - variant: Any ``LiquidGlassVariant`` (0-19). Defaults to `.v11`.
    ///   - cornerRadius: Corner radius in points. Defaults to `28`.
    ...
    private func callPrivateVariantSetter(on object: AnyObject, value: Int) {
        guard
            ...
            print("[LiquidGlass] selector set_variant: not found, using default")
            return
        }
    ...
    public func makeNSView(context: Context) -> NSView {
        ...
        if let glassType = NSClassFromString("NSGlassEffectView") as? NSView.Type {
            print("[LiquidGlass] Found NSGlassEffectView, using variant \(variant.rawValue)")
            ...
            // Reduce shine opacity on highlight layers
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                if let sublayers = glass.layer?.sublayers {
                    for sublayer in sublayers {
                        let classString = NSStringFromClass(type(of: sublayer))
                        // Reduce opacity of shine/highlight layers instead of hiding them
                        if classString.contains("CALayer") && !classString.contains("Backdrop") && !classString.contains("NSHosting") && !classString.contains("Content") {
                            sublayer.opacity = 0.3  // Reduce shine to 30% opacity
                        }
                    }
                }
            }
        ...
        print("[LiquidGlass] NSGlassEffectView not found, falling back to NSVisualEffectView")
